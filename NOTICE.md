...
hub is MIT licensed. It depends on, and (when packaged) redistributes, the
...
| [CPython](https://github.com/python/cpython) (via [python-build-standalone](https://github.com/astral-sh/python-build-standalone)) | PSF-2.0 | the interpreter bundled inside Hub.app |

hub *drives* these, and does not redistribute them:

- **Claude Code** and **Codex** — run as the user's own installed CLIs, with the
  user's own subscription. hub never reads, copies or reuses their credentials,
...
- **mlx_lm**, **MLX** (MIT) and **ComfyUI** (GPL-3.0) — run as separate local
  servers the user installs. hub starts and stops them and speaks HTTP to them.
- Model weights — downloaded from Hugging Face under each model's own licence,
  which hub shows before downloading.

Names like Claude, Codex, OpenAI, Anthropic and Hugging Face are trademarks of
their owners. hub is not affiliated with, endorsed by, or built by any of them.
