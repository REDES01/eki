...
def plist_for(root: Path) -> dict:
    ...
    return {
        "Label": LABEL,
        "ProgramArguments": [str(python), "-m", "hub.cli", "serve"],
        ...
        # launchd's PATH is /usr/bin:/bin. The CLIs hub drives live in these,
...
def install(root: Path) -> str:
    ...
    if not python.exists():
        return f"no virtualenv at {python} — run ./hub.sh once first"
...
def status() -> str:
    if not installed():
        ...
        if code == 0:
            ...
            return f"installed by Hub.app · {state}"
        return "not installed — `hub agent install` to start the engine at login"
    if not loaded():
