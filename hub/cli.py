"""hub — tell it to do something; it keeps doing it until it's done.

    hub ask "why is this slow?"          routed; the answer streams here
    hub ask "..." --backend claude       force a backend
    hub ask "..." --repo .               may edit files there
    hub ask "a brass compass" --image    draw it
    hub ask "..." --continue             same thread as last time
    hub ask "..." --detach               start it and return straight away

Every ask is a run in the engine, not in this terminal: Ctrl-C stops
*watching*, never the work. Pick it back up with `hub watch <run>`.

    hub runs                             what's running and what finished
    hub watch <run>                      follow one, from the start
    hub cancel <run>                     stop it (its CLI child dies with it)
    hub diff <run>                       what it changed in the repo

    hub history [-q text]                conversations, or a search of them
    hub show <conversation>              one thread, with who answered
    hub cost <conversation>              who answered, and what they reported
    hub backends | models | policy       what exists, what's loaded, the rules
    hub agent install|uninstall|status   run the engine from login, always
    hub serve                            run the engine in the foreground
"""
...
def ensure_engine(service: str) -> None:
    ...
    if agent.installed():
        ...
        subprocess.Popen(
            [sys.executable, "-m", "hub.cli", "serve"], cwd=str(ROOT),
    ...
    for _ in range(50):
        ...
        if engine_up(service):
            return
    print(f"! the engine didn't come up on {service} — try `hub serve` to see why",
          file=sys.stderr)
    raise SystemExit(1)
...
def watch(rid: str, service: str, quiet: bool = False, conversation: str = "") -> int:
    ...
    try:
        ...
        print(f"\n[let go — it's still running: hub watch {rid}]", file=sys.stderr)
        return 0
...
def cmd_agent(args) -> int:
    if args.action == "install":
        ...
        subprocess.run(["pkill", "-f", "hub.cli serve"], capture_output=True)
        time.sleep(0.5)
