...
runtime hub manages, start it once, and measure it with a one-line smoke
...
async def search(query: str = "", limit: int = 30) -> List[Dict[str, Any]]:
    ...
    for m in r.json():
        ...
        # hub serves chat models; speech, embedding and image repos don't belong here
...
def write_scripts(key: str, repo: str, port: int, samp: Dict[str, Any],
    ...
    """`thinking=False` turns a reasoning model's thinking off — hub's
...
# Written by hub. Serves {repo} on 127.0.0.1:{port}.
...
# Written by hub. Stops whatever listens on {port}.
