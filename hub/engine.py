...
`hub ask` in a terminal and the Mac app are two views of the same engine —
...
class Engine:
    ...
    def _is_up(self, key: str) -> Optional[bool]:
        ...
        if model.running:
            return True
        # stopped, but hub can bring it up: still a candidate, started on
    ...
    async def deploy(self, repo: str, label: str = "") -> Dict[str, str]:
        rid = self.runs.create(f"Set up {repo}", requested="hub", kind="deploy",
    ...
    async def _deploy(self, run: Dict[str, Any]) -> AsyncIterator[Union[str, Dict[str, str]]]:
        ...
        repo = job["repo"]
        yield {"backend": "hub", "reason": f"setting up {repo}"}
        ...
        if not model or not model.running:
            raise BackendError(f"{message} — see ~/.hub/models/{key}/server.log")
        try:
        ...
        yield (f"It answered in {result['seconds']} s{speed}:\n\n> {result['reply']}\n\n"
               f"Ready. hub will route to it when it's the right fit, and unload it "
