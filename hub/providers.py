"""Providers: everything hub can send work to, stored in the database.

A provider is one record that serves as a routing backend, a quota source
if it has one, and, for a local server, a runtime hub can start and stop.
...
class Provider:
    ...
    #: a local server hub manages: {port, start, stop, gb, idle_minutes, kind}
