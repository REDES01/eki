...
to the CLI. hub starts it read-only, does the JSON-RPC handshake, asks once,
...
def parse(payload: Any, now: int) -> Reading:
    ...
    if not windows:
        raise ValueError("codex app-server returned a shape hub doesn't recognise")
